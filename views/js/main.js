...
var resizePizzas = function(size) {
  ...
  // This pulls variables out of the for loop
...
//Variable moved outside the loop
...
//Function calls "requestAnimationFrame"
...
//callback function, attached to event listener
function onScroll() {
  scrollPosition = window.scrollY;
  callAnimation();
}

//Pizza positions updated
...
//Variables out of for loop
  ...
  var currentScrollPosition = scrollPosition / 1250;
  var phase;
  for (var i = 0; i < items.length; i++) {
    phase = Math.sin(currentScrollPosition + (i % 5));
...
// binds event listener to onScroll function
window.addEventListener('scroll', onScroll);

// Generates the sliding pizzas when the page loads.
//Variables created and moved outside the loop
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  //s is 256 pixels per row
  var s = 256;
  //number of pixels in screen height divided by pixels/row equals number
  //of rows. Resultant pizzas to be displayed is product of rows * columns
  var row = screen.height / s;
  var result = cols * row;
  var elem;
